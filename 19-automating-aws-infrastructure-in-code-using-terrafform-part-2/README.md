...
+ IP routing is the process of sending packets from a host on one network to another host on a different remote network Internet Gateways An internet gateway is a horizontally scaled, redundant, and highly available VPC component that allows communication between your VPC and the internet
...
+ Create a file called `route_tables.tf` and entering the following codes to create routes for both public and private subnets:
...
+ Create a file named `roles.tf` and entering the following codes:
...
+ Create an IAM policy which allows an IAM role to perform action describe to EC2 instances:
...
+ Create a new file called `security.tf` and entering the following commands to create security group for the Internal and External load balancer, the bastion server, Nginx server, the tooling and wordpress webserver and the data layer:
...
+ Enter the following codes to create external(internet-facing) load balancer which balances traffic for the Nginx servers:
...
+ Enter the following codes which creates notification for all the auto scaling group:
...
+ Create a new file called `asg-wordpress-tooling.tf` and enter the following codes which creates launch templates and Auto Scaling Group for both the wordpress and tooling webserver:
...
+ Create new file called `efs.tf`.

+ Enter the following code to create KMS key to encrypt EFS resource:
...
+ Create new file called `rds.tf` and enter the following code which creates MySQL Relational Database System:
